...
import { equals, parseDateFields } from '../utils/utils';
import { constantValues } from "../app.constantValues";

declare const $: any

@Injectable()
export class DataTableExtensionsService {

  constructor(private http: HttpClient, private toastr: ToastrService, private constantValues: constantValues) {
  ...
  init() {
      ...
      var conf = $.extend({
        ...
        beforeSend: function (xhr: any) {
          ...
          if (sAuth) {
            ...
            xhr.setRequestHeader('currentTimeZone', self.constantValues.currentTimeZone)
          } else {
            // ir para login???
          }
        },
    ...
    class SpecialColumn {
      ...
      public destroy() {
        ...
        this.wrapper && this.wrapper.off({
          'show.bs.dropdown': this.onShowDropdown,
          'hide.bs.dropdown': this.onHideDropdown
        })
      }
      ...
      cfg = defaultsDeep(cfg, {
        ...
        export: () => {
          ...
          if (!data.length) {
            ...
            self.toastr.error(cfg.dt.i18n('sZeroRecords'))
            ...
            if (dataCache.recordsFiltered <= data.length) {
              ...
              const fromRemote = () => {
                ...
                self.http.get<any>(ctx.url, {params: $.param(aData)}).subscribe(r => {
                  dataCache.exportData = r.data
                  dataCache.exportParams = aData
                  action()
                })
